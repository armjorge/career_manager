 ...
 * automatically when enable_frontend = true (no manual second apply for that).
 */
...
terraform {
  ...
  #   key            = "career-app/dev/terraform.tfstate"
  #   region         = "us-east-1"
...
locals {
  ...
  # Extra origins from tfvars (localhost / custom domains) + CloudFront when present.
